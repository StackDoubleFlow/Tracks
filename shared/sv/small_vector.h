// taken from https://github.com/KonanM/small_vector/blob/master/include/small_vector/small_vector.h
...
template <typename T, size_t MaxSize = 8, typename NonReboundT = T> struct small_buffer_vector_allocator {
  ...
  template <class U>
  constexpr small_buffer_vector_allocator(small_buffer_vector_allocator<U, MaxSize, NonReboundT> const&) noexcept {}

  template <class U> struct rebind { typedef small_buffer_vector_allocator<U, MaxSize, NonReboundT> other; };
  // don't copy the small buffer for the copy/move constructors, as the copying is done through the vector
  constexpr small_buffer_vector_allocator(small_buffer_vector_allocator const& other) noexcept
      : m_smallBufferUsed(other.m_smallBufferUsed) {}
  constexpr small_buffer_vector_allocator& operator=(small_buffer_vector_allocator const& other) noexcept {
    m_smallBufferUsed = other.m_smallBufferUsed;
    return *this;
  }
  constexpr small_buffer_vector_allocator(small_buffer_vector_allocator&&) noexcept {}
  constexpr small_buffer_vector_allocator& operator=(small_buffer_vector_allocator const&&) noexcept {
    return *this;
  }
  ...
  constexpr void deallocate(void* p, const size_t n) {
    ...
    if (&m_smallBuffer != p) m_alloc.deallocate(static_cast<T*>(p), n);
    m_smallBufferUsed = false;
  }
  // according to the C++ standard when propagate_on_container_move_assignment is set to false, the comparision
  // operators are used to check if two allocators are equal. When they are not, an element wise move is done instead of
  // just taking over the memory. For our implementation this means the comparision has to return false, when the small
  // buffer is active
  friend constexpr bool operator==(small_buffer_vector_allocator const& lhs, small_buffer_vector_allocator const& rhs) {
...
template <typename T, size_t N = 8> class small_vector : public std::vector<T, small_buffer_vector_allocator<T, N>> {
public:
  ...
  small_vector(small_vector const&) = default;
  ...
  template <class InputIt> small_vector(InputIt first, InputIt last) : small_vector() {
