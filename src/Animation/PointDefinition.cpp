...
struct TempPointData {
  ...
  TempPointData(sbo::small_vector<float, 5> copiedList, float time, Functions easing, bool spline)
      : copiedList(std::move(copiedList)), time(time), easing(easing), spline(spline) {}

  explicit TempPointData(sbo::small_vector<float, 5> copiedList, float time)
...
PointDefinition::PointDefinition(rapidjson::Value const& value) {
  ...
  for (int i = 0; i < value.Size(); i++) {
    rapidjson::Value const& rawPoint = value[i];
    ...
    if (rawPoint.IsArray()) {
      ...
      for (int j = 0; j < rawPoint.Size(); j++) {
        rapidjson::Value const& rawPointItem = rawPoint[j];

        if (rawPointItem.IsNull()) continue;
